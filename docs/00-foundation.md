...
- **Судья ≠ генератор.** LLM-судья Ragas — из ДРУГОГО семейства, чем генератор (генерируем `gpt-4o-mini` → судим моделью другого вендора/локальной), иначе self-preference bias завышает оценки. Отдельная `judge_model` в конфиге (провайдеро-агностичный слой §2.5 это позволяет).
